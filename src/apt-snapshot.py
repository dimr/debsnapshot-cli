...
parser.add_argument('-alls', '--all-source-for-package-version', help='list all source files associated with this package at that version', action='store_true')
parser.add_argument('-alb', '--all-binpackages-for-package-version', help='list all binary packages associated with this source package at that version', action='store_true')
...
if args.all_source_versions:
    ...
    print package.list_all_sources_for_this_package_at_version(args.version[0])
    ...
    if args.architecture is None:
        print package.list_all_files_associated_with_this_source_package_at_that_version(args.version[0])
        exit()
    # arch = getattr(args, 'all_files')[1]
    pprint.pprint(package.list_all_files_associated_with_this_source_package_at_that_version(args.version[0], args.architecture[0]), width=1)
    ...
    pprint.pprint(package.info_from_hash(args.version[0], args.architecture[0]), width=1)
